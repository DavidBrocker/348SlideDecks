...
::: hero-def
[Mean]{.term-pill}
: The average of all $x$ values -- best used with a normal distribution.
  Formula: $\dfrac{\sum(x)}{n}$
:::
...
::: hero-def
[Median]{.term-pill}
: The middle value in **sorted** data -- best used with a skewed
  distribution (is the data pulled to the left or the right?).
  Formula: $Med(X) = \dfrac{n + 1}{2}$
:::
...
::: hero-def
[Mode]{.term-pill}
: The most common value (or values, in bi/tri-modal distributions) --
  shows us where the data is clustered.
:::
