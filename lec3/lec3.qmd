...
<!-- Trial: live iframe of the linked quincunx simulator, replacing the
     dead images/galton.png reference. Untested against the site's own
     framing policy -- if it renders blank/blocked, fall back to the
     images/galton.png treatment (needs a real image) or just the link. -->
<iframe src="https://www.mathsisfun.com/data/quincunx.html" width="900" height="560" style="border: 2px solid #8FAF9E; border-radius: 16px;"></iframe>
...
::: callout-tip
1. Sum all x values: $x = 4, 2, 5, 6$
2. Divide by the number of x values ($n$)
3. $\dfrac{\sum(x)}{n} = \dfrac{4+2+5+6}{4} = \dfrac{17}{4} = \color{red}{4.25}$
:::
...
::: callout-tip
1. Put all x values in order from smallest to largest
2. Find the middle number (average the 2 middle numbers if $n$ is even)
3. $x = \{6,2,4,2,8\} \rightarrow \{2,2,4,6,8\} \rightarrow \text{median} = \color{blue}{4}$
:::
...
::: callout-tip
1. Put all x values in order and count how often each one repeats
2. $x = [3,2,1,5,3,7,8,3,2,4,1] \rightarrow [1,1,2,2,\color{green}{3},\color{green}{3},\color{green}{3},4,5,7,8]$
3. The mode is whichever value repeats the most -- here, $\color{green}{3}$ (there can be more than one mode)
:::
...
mode.R
```r
x <- c(3, 2, 1, 5, 3, 7, 8, 3, 2, 4, 1)

# base R has no mode() function for statistical mode --
# mode() actually returns an object's storage type, a classic gotcha
table(x)
