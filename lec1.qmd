...
C - Maybe we can make this more profile-y with 7 in a circle, school icon with Brooklyn next to it and maybe FSC as well to add that tie-in, for research let's add a coffin icon
...
- **Interactive:** Whenever possible, I favor dynamic content over static content
  ...
  - C - this should have something that emphasizes that, maybe even a :hover effect for that specific point
...
C - I wonder if adding in the same marquee effect from my website would work or just adding it as an iframe
...
-c I'd like to redo this so that it looks a bit more coherent, modern, etc.
...
C - lets include a more detailed chart or use Mermaid Viz
...
C- Maybe let's try and find a better example of this
...
C - I like these points but I think they might need some more context, I do cover it in class but at first glance it can be difficult to know what I mean
...
- **Labs: Let's add in a descriptor about labs, aim, goal, etc.**
...
C - Let's convert this to a flow chart or something that 'moves' from step to step
...
C - aim to add an animation of a book opening

Overall Comments: I think this lecture just needs a bit of polishing. I don't think there are any content gaps or areas that need amending.