...
::: {.column width="33%"}
- One-Sample T-Test
  - $t=\frac{\bar{x}-\mu}{\sigma/\sqrt{n}} | \frac{\bar{x}-\mu}{s/\sqrt{n}}$
:::
...
$\large{t = \frac{\color{red}{\bar{x}}-\color{blue}{\mu}}{\color{orange}{s/\sqrt{n}}}}$
:::
...
- $\color{orange}{s/\sqrt{n}}$ = the standard error (SE) — estimated from the sample, since population $\sigma$ is typically unknown
...
> A one-sample t-test was conducted to determine whether students’ exam scores differed from the national average ($\mu$ = 75). Results were not statistically significant, ( t(24) = 1.5, p \> .05 ), indicating that students performed similarly to the national average.
