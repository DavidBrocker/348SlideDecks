...
-   We've been writing the sample mean as $\bar{x}$ — you'll keep using that.

-   You'll also see the sample mean written as $M$ in published papers — that's the [APA style](https://apastyle.apa.org/) convention for reporting results, not a different calculation. $\bar{x}$ and $M$ mean the same thing.
...
-   The Standard Error is **not** just another name for $\sigma$ — it's the standard deviation of the *distribution of sample means*, which is why it shrinks as $n$ grows ($SE = \sigma / \sqrt{n}$).

-   In other words: $\sigma$ describes how spread out individual scores are; $SE$ describes how spread out sample means are.
...
-   Under H₀, there are ${8 \choose 4} = \frac{8!}{4!(8-4)!} = 70$ equally likely ways to guess which 4 cups are milk-first.
-   So the probability of guessing all 8 correctly by chance is $\frac{1}{70} \approx 1.4\%$.

Since 1.4% is less than 5%, it’s rare enough to count as evidence against H₀.
...
-   Null Hypothesis: There is no difference in the DV between the IV groups.
...
-   *Brendan rejects the null hypothesis that states there is no difference in death anxiety between participants who view their own face as deceased and those who only view a face on a mortuary table.*
