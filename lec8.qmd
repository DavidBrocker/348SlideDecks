...
set.seed(101)
df_sampling <- tibble(
  value = c(rnorm(500, mean = 6.4, sd = 1.2),
            rnorm(500, mean = 6.8, sd = 1.2)),
...
-   Under H₀:\
    $(P(\text{8 correct}) = 1/70 ≈ 0.014)$

Since this is less than 5%, it’s rare enough to count as evidence against H₀.

## Decision & Interpretation

-   **If 8 / 8 correct → Reject H₀** → evidence she can tell the difference.\
-   **Otherwise → Fail to reject H₀** → no convincing evidence of ability.
